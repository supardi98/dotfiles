...
# Quick instant mode: area copy
take_copy_area() {
    grimblast --notify copy area
}

# Quick instant mode: full copy
take_copy_full() {
    grimblast --notify copy screen
}

# Quick instant mode: window copy
take_copy_window() {
...
if [[ "$1" == "--instant" ]]; then
    ...
    take_instant_area
    exit 0
elif [[ "$1" == "--copy-area" ]]; then
    take_copy_area
    exit 0
elif [[ "$1" == "--copy-full" ]]; then
    take_copy_full
    exit 0
elif [[ "$1" == "--copy-window" ]]; then
    take_copy_window
    exit 0
fi
