...
# Load Settings
if [ -f ~/.config/hypr/settings/wallpaper-folder ]; then
    SEARCH_PATH=$(cat ~/.config/hypr/settings/wallpaper-folder)
fi
...
if [ -z "$LIST" ]; then
    ...
    if [ ! -d "$SEARCH_PATH" ]; then
        ...
        echo ":: You can define your own Wallpaper folder in ~/.config/hypr/settings/wallpaper-folder"
    else
